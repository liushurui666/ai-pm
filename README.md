...
AI_MODELS=qwen3.6-plus,qwen3.6-max-preview,qwen3.6-flash,deepseek-v4-pro,deepseek-v4-flash,kimi-k2.6,glm-5.1,glm-4.5-air,MiniMax-M2.7,qwen-plus,qwen3.7-max
```

`AI_BASE_URL` 默认使用 DashScope OpenAI-compatible 兼容接口，也可以替换成其他兼容服务的 base URL。`AI_MODELS` 是前端可切换模型白名单，服务端会做短超时 tools 兼容探活并缓存结果，探活失败的模型不会出现在 ChatBox 下拉里；未显式配置时默认候选对齐 ai-interview 的百炼模型清单：`qwen3.6-plus,qwen3.6-max-preview,qwen3.6-flash,deepseek-v4-pro,deepseek-v4-flash,kimi-k2.6,glm-5.1,glm-4.5-air,MiniMax-M2.7`，并保留 AI PM 历史候选 `qwen-plus,qwen3.7-max`。配置后 `/api/assistant` 会以 AI SDK tools 读取当前项目、任务、风险、需求、版本和成员负载等上下文，由模型基于工具结果生成流式多轮回复；周报导出接口保留本地模板兜底，避免影响下载体验。
